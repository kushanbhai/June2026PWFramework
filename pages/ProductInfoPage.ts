...
export class ProductInfo {
  ...
  async getProductImagesCount(): Promise<number> {
    ...
    console.log(
      `the total no of images for ${this.productHeader()} is ==>> ${imageCnt}`,
    );
    return imageCnt;
  }
  /**
  ...
  private async getProductMetaData() {
    const productMetaData = await this.productMetaData.allInnerTexts();
  ...
  private async getProductProductPriceData() {
    const productPriceData: string[] =
      await this.productPriceData.allInnerTexts();
    const productPrice = productPriceData[0].trim();
    const productExTax = productPriceData[1].split(":")[1].trim();
