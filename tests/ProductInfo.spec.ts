...
test(`verify product header for product: ${product.productName}`,{
    tag:['@product','@sanity', '@regression'],
   annotation: [ // add additional info to your test good practice, it will be easier for everyone in the team
    {type: 'epic',description: 'Epic 100- design product page for open cart app'},
    ...
    let productInfo: ProductInfo = await resultsPage.selectProduct(product.productName);
...
for (let product of search){
test(`verify product images for product ${product.productName}: ${product.imagecount}`,{tag: ['@product','@sanity']},async({homePage})=>{
    let resultsPage:ResultsPage = await homePage.doSearch(product.searchKey);
    let productInfo: ProductInfo = await resultsPage.selectProduct(product.productName);
...
test('verify product metadata',async({homePage})=>{
    ...
    let productInfo: ProductInfo = await resultsPage.selectProduct('MacBook Pro');
